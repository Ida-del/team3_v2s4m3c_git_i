...
public class OrderreqCont {
  ...
  // 회원페이지: 목록
  @RequestMapping(value = "/orderreq/list_all.do", method = RequestMethod.GET)
  ...
  public ModelAndView list_by_userid_search(HttpSession session, OrderreqVO orderreqVO,
                                                            @RequestParam(value="eb_title", defaultValue = "") String eb_title) {
    ModelAndView mav = new ModelAndView();
    String user_id=(String)session.getAttribute("id");
    ...
    if(user_id == null) {
      ...
      hashMap.put("user_id", user_id);    // #{user_id}
      hashMap.put("eb_title", orderreqVO.getEb_title());    // #{eb_title}
      
      // 사용자별 검색 목록
      List<OrderreqVO> list = this.orderreqProc.list_by_userid_search(hashMap);
      mav.addObject("list", list);  
      
      // 사용자별 검색 레코드 수
      int search_count = this.orderreqProc.search_count(hashMap);
